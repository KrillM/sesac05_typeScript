...
type ConditionType = "writer" | "title";

const Tsc = () => {
  const [inputWriter, setInputWriter] = useState<string>("");
  // 초기값을 넣어줄 경우 초기값의 타입으로 알아서 해석한다.
  ...
  const [searchType, setSearchType] = useState<ConditionType>('writer')
  ...
  const selectSearchType = (e: ChangeEvent<HTMLSelectElement>) => {
    ...
    // select option 상태 설정
    setSearchType(e.target.value as ConditionType)
  }
  ...
  return (
    <div>
      <form>
        ...
        <input id="wirter2" type="text" name="writer" value={inputWriter} onChange={(e) => setInputWriter(e.target.value)}/>
        <label htmlFor="title2">제목:</label>
        <input id="title2" type="text" name="title" value={inputTitle} onChange={(e) => setInputTitle(e.target.value)}/>
      ...
      <form>
        ...
        <input type="text" name="search" placeholder="검색어" value={inputSearch} onChange={(e) => setInputSearch(e.target.value)}/>
